(ns citrusops.operation
  "OperationActor -- one citrus orchard operation = one supervised actor
  run. The advisor (CitrusOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Citrus Operations
  ...
  Governor is clean -- see `citrusops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `orchardops.operation`, cloud-itonami-isic-0122). This stub version
  ...
  (:require [citrusops.advisor :as advisor]
...
(defn run-operation
  "Run one citrus orchard operation through the advisor -> governor ->
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- a `citrusops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
