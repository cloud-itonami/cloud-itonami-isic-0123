(ns citrusops.phase
  "Phase gate for the citrus orchard operations coordinator rollout (0->3
  maturity). Mirrors `orchardops.phase` (cloud-itonami-isic-0122) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`citrusops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [citrusops.governor :as governor]))
